...
function handleAskQuestion() {
    ...
    addBotMessage('Si je ne suis pas en mesure de te donner une réponse immédiate, je vais contacter l\'entreprise afin de trouver une solution à ta question.');
...
function handleUserFirstname() {
    ...
    if (userFirstname) {
        ...
        // Vérifier si le débiteur est présent dans le fichier Excel
        fetch('http://127.0.0.1:8000/api/verify_user', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ first_name: userFirstname, last_name: document.getElementById('user-name').value.trim() })
        })
...
function askQuestion(question) {
    ...
    fetch('http://127.0.0.1:8000/api/chat', {
        ...
        body: JSON.stringify({ message: question, first_name: document.getElementById('user-firstname').value.trim(), last_name: document.getElementById('user-name').value.trim(), session_id: session_id })
    })
    .then(response => response.json())
    .then(data => {
        if (data.session_id) {
